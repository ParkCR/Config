...
// @version      2.1
// @description  在网页中将以指定前缀开头的图片URL进行替换，支持多个CDN节点选择，优化图片加载速度，完美适配手机电脑两段
...
(function () {
    ...
    const prefixRegex = /^https:\/\/[^\/]+\.[^\/]+\/(wp-content\/uploads|uploads\/posts|pics\/post)\/.*/;
    ...
    modal.appendChild(accelerateButton);
    modal.appendChild(fullAccelerateButton);
    modal.appendChild(removeAccelerationButton);
    ...
    function handleDragStart(e) {
        ...
        offsetX = event.clientX - dragButton.offsetLeft;
        offsetY = event.clientY - dragButton.offsetTop;
    }
    ...
    dragButton.addEventListener('touchstart', handleDragStart);
    ...
    dragButton.addEventListener('click', handleClick);
    dragButton.addEventListener('touchend', handleClick);
