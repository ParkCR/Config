...
// @version      1.7
// @description  在网页中将以指定前缀开头的图片URL进行替换，支持多个CDN节点选择，优化图片加载速度，并提供“加速全部图片”功能
...
// @grant        none
// @changelog    1.7: 优化性能，可自选部分加速还是全部加速
...
(function () {
    ...
    fullAccelerateButton.style.marginLeft = '10px'; // 添加一些间距
    ...
    container.appendChild(accelerateButton);
    container.appendChild(fullAccelerateButton);
    ...
    fullAccelerateButton.addEventListener('click', function () {
        const selectedNode = select.value;
        forceProcessAllImages(selectedNode);
    });
